...
public struct GenerateMazeJob : IJob
{
	...
	public void Execute()
	{
		...
		var scratchpad = new NativeArray<(int, MazeFlags, MazeFlags)>(4, Allocator.Temp, NativeArrayOptions.UninitializedMemory);

		var activeIndices = new NativeArray<int>(maze.Length(), Allocator.Temp, NativeArrayOptions.UninitializedMemory);
	...
	/**
	 * Sucht nach leeren Nachbarzellen (nicht diagonal) und speichert diese in >>scratchpad<< zwischen. 
	 * 
	 * 
	 */
